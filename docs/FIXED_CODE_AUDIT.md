...
~~- **Статус:** Закрыто — CDN-скрипты доверенные, риск принят.~~
...
- **Исправлено 2026-07-29:** `attachFreshSignedUrls` теперь проверяет `created_at` + `SIGNED_URL_TTL`: если stored `file_url` ещё жив, API-вызов пропускается.
...
- **Проверено 2026-07-29:** `renderSplitReceipts` всегда делает `innerHTML`, который уничтожает старый DOM вместе со слушателями. Накопления нет — ложное срабатывание.
...
- **Исправлено 2026-07-29:** `catch` уже содержит `showToast` с сообщением об ошибке.
...
- **Исправлено ранее:** Вызов удалён из кода, аудит устарел.
...
- **Проверено 2026-07-29:** JS (дефолт `0`) и HTML (placeholder `0`, min `0`) согласованы. Номера строк в audit устарели.
...
- **Исправлено ранее:** Функция удалена (была мёртвой — нигде не импортировалась).
...
- **Скипнуто 2026-07-29 (MVP):** Старые чеки удаляются вручную, не чинить кодом.
...
- **Скипнуто 2026-07-29 (MVP):** Текущий `encodeURIComponent` уже корректно экранирует разделители. Версионирование не нужно до выхода в прод.
...
- **Исправлено 2026-07-29:** Проверка прав добавлена на сервере.
...
- **Скипнуто 2026-07-29 (MVP):** Ротация ключей не нужна на этапе MVP.
...
- **Исправлено 2026-07-29:** `cleanupInterval` сохранён, `clearInterval` перед перезапуском и на `beforeunload`.
...
- **Скипнуто 2026-07-29 (MVP):** createClient дёшев, не влияет на производительность.
...
- **Исправлено ранее:** Переменные удалены из кода.
...
- **Исправлено ранее:** Fallback `icons[type] || icons.info` на месте.
...
- **Исправлено 2026-07-29:** Капча проверяется первой в обоих формах (логин и сброс пароля).
...
- **Проверено 2026-07-29:** По дизайну Supabase — anon-ключ публичен, RLS решает. Ок.
...
- **Скипнуто 2026-07-29 (MVP):** SQL-функции не в репозитории, как и остальные RPC.
...
- **Проверено 2026-07-29:** Guard `!parsed.signature || !parsed.taxId → showResult('invalid')` уже есть, мусорные QR не проходят.
