...
| ~~G3~~ | ~~`verify.js`~~ | ~~745~~ | ~~`Chart` — ReferenceError при недоступности CDN (ошибка аудита — Chart нет в verify.js, в business-panel.js уже защищён)~~ |
| ~~G4~~ | ~~`receipt-generator.js`~~ | ~~2, 41~~ | ~~`qrcode`, `jspdf` — уже были guard'ы (`typeof qrcode === 'undefined'`, `typeof window.jspdf === 'undefined'`)~~ |
| ~~G5~~ | ~~`receipts.js`~~ | ~~357, 365~~ | ~~`jspdf`, `qrcode` — динамическая загрузка с `s.onerror` + try/catch~~ |
| ~~G6~~ | ~~`business-panel.js`~~ | ~~745~~ | ~~`Chart` — уже был guard (`if (!window.Chart) return;`)~~ |
| ~~G7~~ | ~~`auth.js`~~ | ~~48, 93, 131, 162, 236, 263~~ | ~~`turnstile` — уже был guard (`typeof turnstile !== 'undefined'` + `waitForTurnstile()` polling)~~ |
| ~~G8~~ | ~~`business-panel.js`~~ | ~~12~~ | ~~`showToast` — fallback-заглушка создаёт контейнер и тост через DOM~~ |
...
|-------|----------|--------|
| ~~`.reveal` / `.reveal.active`~~ | ~~`style.css`~~ | ~~949-958~~ |
...
### ~~4.4 Массовое дублирование CSS-классов между файлами~~ **СКИПНУТО (MVP)**
...
### ~~4.5 Недостающие `backdrop-filter` (только -webkit-)~~ **FIXED**

~~`dashboard.css:981,1200,1658`, `business.css:735,1262`, `style.css:1211,1169` — везде только `-webkit-backdrop-filter`, без стандартного `backdrop-filter`.~~ *(В текущем коде оба свойства уже были, но в 3 местах были разные значения — синхронизированы)*

### ~~4.6 `!important` блокирует `.active`~~ **FIXED**
